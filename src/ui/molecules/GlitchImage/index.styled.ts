...
const Styled = {
	Container: styled.div.attrs((props) => ({ ...props }))`
		width: 100%;
		height: auto;
		position: relative;
		display: flex;
	...
	`,
	GlitchItem: styled.div.attrs(({ background, ...rest }: { background: string }) => ({ background, ...rest }))`
		background: url('${(p) => p.background}') no-repeat 50% 50% / cover;
		width: 100%;
		height: 100%;
		top: 0;
		left: 0;
		...
		&:nth-child(5) {
			...
			@keyframes glitch-anim-flash {
				...
				5% {
					opacity: 0.2;
					transform: translate3d(${gapHorizontal}, ${glitchHeight}, 0);
				}
				5.5%,
				100% {
					opacity: 0;
